...
if f_a * f_b < 0:
    ...
    for n in range(iter):
        ...
        if f_p0 == 0:
            print(f"| {1:^4} | {a:^15.8f} | {b:^15.8f} | {p0:^15.8f} | {mult:^15.8f} | {f_p0:^15.8f} |")
            break
        if f(a) * f_p0 < 0:
            b = p0
        else:
            a = p0
    print("="*98)
    print(f"\n Raiz encontrada en {p0} \n")
    print(f"Evaluacion de la raiz en la funcion f(x): {f_p0} \n")
else:
    print("="*60)
    print("ERROR: La funcion debe de tener signos opuestos al evaluar en los extremos del intervalo.")
    print("="*60)